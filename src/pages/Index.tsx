  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('trains').select('*').order('train_number');
  ...
  return <>
      ...
      <div className="container mx-auto px-4 py-12 max-w-6xl">
        ...
        {isLoading ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {trains?.map(train => <Card key={train.id} className="cursor-pointer hover:shadow-lg transition-shadow border-border/50" onClick={() => navigate(`/train/${train.train_number}`)}>
                <CardHeader>
                ...
                <CardContent className="space-y-2">
                  ...
                  {train.current_station && <div className="flex items-center gap-2 text-sm text-muted-foreground">
